# Gonana Crypto Payment

Gonana crypto payment is a customized DApp written in Python, which originally resembles the one provided by the sample [Echo Python DApp](https://github.com/cartesi/rollups-examples/tree/main/echo-python).
...
```shell
DAPP_NAME=payment docker compose -f ./deploy-testnet.yml up
```

This will create a file at `./deployments/<network>/payment.json` with the deployed contract's address.
Once the command finishes, it is advisable to stop the docker compose and remove the volumes created when executing it.

```shell
DAPP_NAME=payment docker compose -f ./deploy-testnet.yml down -v
```
...
```shell
yarn start input send --payload "Hello there" --addressFile path/to/payment/deployments/<network>/payment.json
```
...
ROLLUP_HTTP_SERVER_URL="http://127.0.0.1:5004" python3 payment.py
```
...
```shell
ls *.py | ROLLUP_HTTP_SERVER_URL="http://127.0.0.1:5004" entr -r python3 payment.py
```
